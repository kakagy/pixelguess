...
/** Get a demo puzzle based on the current day (cycles through available puzzles) */
export function getDemoPuzzleForToday() {
  const dayOfYear = Math.floor(
    (Date.now() - new Date(new Date().getFullYear(), 0, 0).getTime()) / 86400000
  );
  const index = dayOfYear % demoPuzzles.length;
  return demoPuzzles[index];
}
